...
def create_table2(table_name,sql): #what are table_name and sql?
    with sqlite3.connect(DATABASE) as db: # this allows for the function to be stored in a variable so that it is easier to write for the user.
        print(table_name) #this prints the name of the database
        cursor = db.cursor() # this enables traversal over the records in a database.
        cursor.execute("select name from sqlite_master where name=?",(table_name,)) # This method executes the database, which the name is given by sqlite_master
...
def create_table3(table_name,sql): #what are table_name and sql?
    with sqlite3.connect(DATABASE) as db: # this allows for the function to be stored in a variable so that it is easier to write for the user.
        print(table_name) #this prints the name of the database
        cursor = db.cursor() # this enables traversal over the records in a database.
        cursor.execute("select name from sqlite_master where name=?",(table_name,)) # This method executes the database, which the name is given by sqlite_master
...
def display_menu():
    ...
    print()
    print("1. (Re)Create Country Table")
    print("2. Add new country")
    print("3. Edit existing country")
    print("4. Delete existing country")
    print("5. Search for countries")
...
def insert_data(values):
    sql = "insert into Country (country,Capital,TimeZone,Population,Nationallanguage,currency) values(?,?,?,?,?,?)"
    query(sql,values)
...
def select_all_countries():
    ...
    return query_with_results(sql2,None)

    sql3 = "select * from Matrix"
    return query_with_results(sql3,None)
...
def main():
    ...
    while not finished:
        ...
        if choice == "1":
            choice2 = input("Select a number between 1-3 for the type of table to be created")
            if choice2 == "1":
             ...
             create_table("Country",sql)
        if choice2 == "2":
             sql2 = """create table Package
             (PackageID integer,
             Country text,
             primary key(PackageID))"""
             create_table2("Package",sql2)
        if choice2 == "3":
            sql3 = """create table Matrix
             (City text,
             London text,
             Paris text,
             Sydney text,
             ...
             Accra, text
             primary key(CountryID))"""
            create_table3("Matrix",sql3)
        elif choice == "2":
            country = input("Please enter name of new country: ")
            Capital = (input("Please enter the capital of the country"))
            TimeZone = input("What is the timezone of the capital")
            Population = input("What is the population of the country")
            Nationallanguage = input("What is the national language of the country")
            currency = input("What is the currency of the country")
            insert_data((country,Capital,TimeZone,Population,Nationallanguage,currency))
        elif choice == "3":
            countries = select_all_countries()
            display_select_results(countries)
            CountryID = int(input("Please enter the id of the product to edit: "))
            country = input("Please enter new name for the Country: ")
            Capital = (input("Please enter the Capital of the country: "))
            TimeZone = input("What is the timezone of the capital")
            Population = input("What is the population of the country")
            Nationallanguage = input("What is the national language of the country")
            currency = input("What is the currency of the country")
            update_country((country,Capital,TimeZone,Population,Nationallanguage,currency,CountryID))
        elif choice == "4":
            countries = select_all_countries()
            display_select_results(countries)
