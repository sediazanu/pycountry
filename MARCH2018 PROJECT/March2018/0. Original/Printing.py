...
def getpackage(Matrix):
    package1 = input("Input the package type you want.")
    print(package1)##This prints out the package inputted so it is easier for the user to remember which package they inputted
    cursor.execute("""SELECT City FROM Package WHERE Package=?""",(package1,)) ##this takes the values where the cities match the package inputted
    ...
    print(array)
    n = input("Which city would you like to start from? (Case sensitive)")##This is used to select the city that the user wants to compare to the rest in the database
